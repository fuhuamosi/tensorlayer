...
def main(_):
    ...
    input_data = tf.placeholder(dtype=tf.int32, shape=(batch_size, num_steps))
    targets = tf.placeholder(dtype=tf.int32, shape=(batch_size, num_steps))
    input_data_test = tf.placeholder(dtype=tf.int32, shape=(1, 1))
    targets_test = tf.placeholder(dtype=tf.int32, shape=(1, 1))

    def inference(x, is_training, steps, reuse=None):
        print("\n num_steps : %d, is_training : %s, reuse : %s" %
              (steps, is_training, reuse))
        initializer = tf.random_uniform_initializer(-init_scale, init_scale)
        with tf.variable_scope('model', reuse=reuse):
            tl.layers.set_name_reuse(reuse)
            network = tl.layers.EmbeddingInputlayer(inputs=x, vocabulary_size=vocab_size,
            ...
            if is_training:
                network = tl.layers.DropoutLayer(network, keep=keep_prob,
                                                 name='drop_embedding')
            network = tl.layers.RNNLayer(layer=network,
    ...
    network_train, lstm1_train, lstm2_train = inference(input_data,
                                                        is_training=True, steps=num_steps,
                                                        reuse=None)
    # Inference for Validating
    network_val, lstm1_val, lstm2_val = inference(input_data,
                                                  is_training=False,
                                                  steps=num_steps, reuse=True)
    # Inference for Testing (Evaluation)
    network_test, lstm1_test, lstm2_test = inference(input_data_test,
                                                     is_training=False, steps=1,
                                                     reuse=True)

    def loss_fn(outputs, target, batch, steps):
        loss = tf.nn.seq2seq.sequence_loss_by_example(logits=[outputs],
                                                      targets=[tf.reshape(target, (-1,))],
                                                      weights=[tf.ones([batch * steps])])
        cost = tf.reduce_sum(loss) / batch
        return cost

    cost_train = loss_fn(network_train.outputs, targets, batch_size, num_steps)
    ...
    with tf.variable_scope('learning_rate'):
        lr = tf.Variable(initial_value=0.0, trainable=False)
    trainable_vars = tf.trainable_variables()
    grads, _ = tf.clip_by_global_norm(tf.gradients(cost_train, trainable_vars),
                                      max_grad_norm)
    ...
    network_train.print_params()
    network_train.print_layers()
    ...
    for i in range(max_max_epoch):
        ...
        train_perplexity = run_epoch(sess, [lstm1_train, lstm2_train], train_data,
                                     train_op, True,
                                     input_data, targets, network_train,
                                     batch_size, num_steps, cost_train, verbose=True)
        print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))
        valid_perplexity = run_epoch(sess, [lstm1_val, lstm2_val], valid_data, tf.no_op(),
                                     False,
                                     input_data, targets, network_val,
                                     batch_size, num_steps, cost_val)
        print("Epoch: %d Valid Perplexity: %.3f" % (i + 1, valid_perplexity))
    test_perplexity = run_epoch(sess, [lstm1_test, lstm2_test], test_data, tf.no_op(),
                                False,
                                input_data, targets, network_test,
                                batch_size, num_steps, cost_test)
    print("Test Perplexity: %.3f" % test_perplexity)


def run_epoch(session, lstms, data, eval_op, is_training,
              input_data, targets, network,
              batch_size, num_steps, cost, verbose=False):
    epoch_size = (len(data) // batch_size - 1) // num_steps
    start_time = time.time()
    costs = 0.0
    iters = 0.0
    states = [tl.layers.initialize_rnn_state(lstm.initial_state) for lstm in lstms]
    for step, (x, y) in enumerate(tl.iterate.ptb_iterator(data, batch_size,
                                                          num_steps)):
        feed_dict = {input_data: x, targets: y}
        for lstm, state in zip(lstms, states):
            feed_dict[lstm.initial_state.c] = state[0]
            feed_dict[lstm.initial_state.h] = state[1]
        if is_training:
            feed_dict.update(network.all_drop)
        fetches = [cost, eval_op]
        for lstm in lstms:
            fetches.extend([lstm.final_state.c, lstm.final_state.h])
        results = session.run(fetches,
                              feed_dict=feed_dict)
        results = list(results)
        _cost = results[0]
        new_states = results[2:]
        for i in range(len(new_states) // 2):
            states.append((new_states[2 * i], new_states[2 * i + 1]))
        costs += _cost
        iters += num_steps
